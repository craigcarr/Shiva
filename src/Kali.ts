...
class Kali {
  ...
  private tempo_best_overlap_position(
    t: tempo_t,
    new_win: Float32Array
  ): size_t {
    ...
    var diff: float;
    var least_diff: float = this.difference(
      new_win.subarray(t.channels * i),
      f,
  ...
  public process(): void {
    ...
    while (t.input_fifo.occupancy() >= t.process_size) {
      ...
      if (!t.segments_total) {
        offset = t.search / 2;
        t.output_fifo.write(
          t.input_fifo.read_ptr(t.channels * offset, t.overlap),
          t.overlap
        );
      } else {
      ...
      t.overlap_buf.set(
        t.input_fifo
          .read_ptr(t.channels * (offset + t.segment - t.overlap))
  ...
  public flush(): void {
    ...
    var remaining: size_t =
      samples_out > t.samples_out ? samples_out - t.samples_out : 0;
  ...
  static segments_pow: double[] = [0, 1, 0.33, 1];
  ...
  public setup(
    ...
    if (segment_ms == null) {
      segment_ms = Math.max(
        10,
        Kali.segments_ms[profile] /
          Math.max(Math.pow(factor, Kali.segments_pow[profile]), 1)
      );
    }
    ...
    t.factor = factor;
    t.segment = handleInt((sample_rate * segment_ms) / 1000 + 0.5);
    t.search = handleInt((sample_rate * search_ms) / 1000 + 0.5);
    t.overlap = Math.max(
      handleInt((sample_rate * overlap_ms) / 1000 + 4.5),
      16
    );
